...
function getOneLogement(int $id): array {
    ...
    $query = "
        SELECT
            ...
            logement.date_creation,
            logement.type_id,
            logement.quartier_id,
            logement.utilisateur_id,
            DATE_FORMAT(logement.date_creation, '%d/%m%/%Y') AS date_creation_format,
            quartier.libelle AS quartier,
            ville.libelle AS ville,
            type.libelle AS type
...
function updateLogement(int $id, string $image, int $type_id): void {
    global $connection;
    
    $query = "UPDATE logement SET image = :image, type_id = :type_id WHERE id = :id";
